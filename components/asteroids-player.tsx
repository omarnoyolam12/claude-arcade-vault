...
declare global {
  interface Window {
    // La expone el fork public/games/asteroids/game.js. Arranca el juego sobre
    // el canvas recibido y devuelve un stop() que cancela el rAF y quita los
    // listeners de teclado.
    startAsteroids?: (canvas: HTMLCanvasElement) => () => void;
    // También la expone el fork mientras hay una partida activa: reinicia el
    // juego sin recrear el canvas (equivale a pulsar Espacio en GAME OVER).
    restartAsteroids?: () => void;
  }
}
...
/**
 * Reproductor jugable de Asteroids: sirve game.js desde /public sobre un
 * <canvas> real dentro del gabinete CRT y sincroniza el HUD React con el estado
...
export function AsteroidsPlayer({ game }: Props) {
  ...
  function handleReady() {
    if (stopRef.current || !canvasRef.current || !window.startAsteroids) return;
    stopRef.current = window.startAsteroids(canvasRef.current);
  }
  ...
  useEffect(() => {
    function onMessage(event: MessageEvent) {
      ...
      if (data.type === "state") {
        ...
        // Reinicio nativo con Espacio: game.js vuelve a "playing" y el modal
        // debe cerrarse para no tapar la partida reiniciada.
  ...
  return (
    <>
      <Script
        src="/games/asteroids/game.js"
        strategy="afterInteractive"
        onReady={handleReady}
      />

      {/* HUD del reproductor — mismo marcado y clases que la maqueta, leyendo
          del estado real del juego. */}
