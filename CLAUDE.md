...
- **`skin-designer`** (`.claude/agents/skin-designer.md`) — subagente que garantiza que cada juego tenga al menos 3 skins seleccionables: **neon**, **retro** y **clásico (default)**. Tiene dos modos: **Modo A (Diseño)**, para un juego sin sistema de skins (hoy, todos), diseña la propuesta concreta de las 3 skins y cierra recomendando levantar una spec vía `/spec`; **Modo B (Verificación)**, para un juego recién portado con `/juego-jugable` + `/spec-impl`, revisa el código real (motor + `components/<slug>-player.tsx`) para confirmar que el selector de skin funciona de verdad, y si no, señala la brecha. No implementa código ni specs; su único output es el informe/veredicto más una entrada en `.claude/skin-designer/registro-skins.md`. El flujo de `/juego-jugable` recuerda invocarlo (Modo B) después de implementar cada juego nuevo. Se invoca de forma explícita ("usa el agente skin-designer…").
